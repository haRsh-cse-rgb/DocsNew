...
export default function JobFilters() {
  ...
  // ✅ Fetch categories and locations like Navbar
  useEffect(() => {
    async function fetchCategoriesAndLocations() {
      try {
        ...
        const uniqueLocations: string[] = Array.from(
          new Set(jobs.map((j: any) => j.location).filter((loc: any): loc is string => typeof loc === 'string' && !!loc))
        );
  ...
  const clearFilters = () => {
    setFilters({ category: '', location: '', batch: '', tags: '' });

    const url = new URL(window.location.href);
    ['category', 'location', 'batch', 'tags'].forEach((key) => url.searchParams.delete(key));
    window.history.pushState({}, '', url.toString());
