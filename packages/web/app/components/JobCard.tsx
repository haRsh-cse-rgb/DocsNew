  ...
  return desc
    ...
    .map((section) => {
      ...
      return {
        heading: rest.length ? heading.trim() : null,
        content: rest.length
          ? rest.join(':').split(',').map((s) => s.trim()).filter(Boolean)
          : [heading.trim()],
      };
    })
    .filter((item) => item.content.length > 0);
...
export default function JobCard({ job }: JobCardProps) {
  ...
  const handleCopy = async () => {
    const message = `${job.companyName} is hiring for ${job.role}${
      job.salary ? ` — Salary: ${job.salary}` : ''
    }.
Apply here: ${window.location.origin}/jobs/${job.jobId}`;
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-3">
            <div>
              <h3 className="text-xl font-semibold text-gray-900 group-hover:text-primary-600 transition-colors duration-200">
                <Link href={`/jobs/${job.jobId}`}>{job.role}</Link>
              </h3>
              <p className="text-lg text-gray-700 font-medium">{job.companyName}</p>
            </div>
            <div className="flex items-center space-x-2">
