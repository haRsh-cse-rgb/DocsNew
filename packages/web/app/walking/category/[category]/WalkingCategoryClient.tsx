...
interface WalkingCategoryClientProps {
  category: string;
}

export default function WalkingCategoryClient({ category }: WalkingCategoryClientProps) {
  ...
  const fetchWalkingByCategory = async () => {
    try {
      setLoading(true);
      const response = await fetch(`https://api.india-jobs.in/api/v1/walking/category/${encodeURIComponent(category)}`);
